function UI(thisNotification: HTMLDivElement, state: INotificationState, { title, message }: INotificationProps, onClose: any) {
    <div bindRef={thisNotification} class={state.style} transition={transition(fly, { duration: 500 })}>
...
export default function _(props: INotificationProps) {
    ...
    onDestroy(() => {
        ...
        dispatch("willUnmount", { height: rect.height, top: rect.top } as INotificationWillUnmountEvent["detail"]);
    });
...
function State({ top, type }: INotificationProps) {
    ...
    state.style = Σ("el-notification", css`top: ${top}px; right: 16px; z-index: 9999`);
