function UI(state: IButtonState, { loading, icon }: IButtonProps) {
    <button class={state.style} onClick>
        <if condition={loading}> <i class={state.loadingStyle} /> </if>
        <if condition={Boolean(icon)}> <i class={state.iconStyle} /> </if>
        <if condition={state.slotExists}> <span> <slot /> </span> </if>
    </button>
}
...
export default function _(props: IButtonProps) {
    const { type = "default", disabled = false, plain = false, icon = "", loading = false, size = "" } = props;
...
function State($$props: SvelteInternalProps, { icon, type, disabled, plain, loading, size }: IButtonProps) {
